...
class FewShotREModel(nn.Module):
    ...
    def ccloss(self, support, proto):
        # Class Cluster Loss
        B = support.size(0)
        N = support.size(1)
        K = support.size(2)
        pos = torch.Tensor(B, N)
        neg = torch.full([B, N, N], 200.0000)
        center = torch.Tensor(B, N)
        mid = torch.Tensor(B, N)
        dis = nn.PairwiseDistance(p=2)
        if K != 1:
            for b in range(B):
                for n in range(N):
                    mid = dis(support[b, n, :], proto[b, n, :])
                    pos[b, n], _ = mid.max(-1)
                    center[b, n] = mid.mean(-1)
                    for m in range(K):
                        if m != n:
                            neg[b, n, m], _ = dis(support[b, m, :], proto[b, n, :]).min(-1)
            neg, _ = neg.min(-1)
            #pos = pos.cuda()
            #neg = neg.cuda()
            #center = center.cuda()
            return (torch.relu(pos - neg + 0.1) + torch.relu(pos - center)).mean()
        else:
            return 0
    ...
    def train(self,
        ...
        for it in range(start_iter, start_iter + train_iter):
            if pair:
                ...
                logits, pred, support, proto = model(support, query, rel_text,
                        N_for_train, K, Q * N_for_train + na_rate * Q)
            loss = model.loss(logits, label) / float(grad_iter) + balance * model.ccloss(support, proto)
            ...
            for it in range(eval_iter):
                if pair:
                    ...
                    if torch.cuda.is_available():
                        ...
                        label = label.cuda()
                    logits, pred, _, _ = model(support, query, rel_text, N, K, Q * N + Q * na_rate)
